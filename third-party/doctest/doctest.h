...
namespace detail {
    ...
    Subcase::Subcase(const String& name, const char* file, int line)
        ...
        if(s->subcasesStack.size() < std::size_t(s->subcase_filter_levels)) {
        ...
        if(s->subcasesStack.size() < std::size_t(s->subcasesCurrentMaxLevel)) {
...
DOCTEST_CLANG_SUPPRESS_WARNING_POP
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        void logTestStart() {
            ...
            for(std::size_t i = 0; i < currentSubcaseLevel; ++i) {
            ...
            if(currentSubcaseLevel != subcasesStack.size()) {
                ...
                for(std::size_t i = 0; i < subcasesStack.size(); ++i) {
                    if(subcasesStack[i].m_name[0] != '\0')
                        s << "  " << subcasesStack[i].m_name << "\n";
                }
            }
...
int Context::run() {
    ...
    if(!testArray.empty()) {
        if(p->order_by.compare("file", true) == 0) {
            ...
            for(std::size_t i = testArray.size() - 1; i > 0; --i) {
